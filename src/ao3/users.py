...
#                'Error logging in to AO3; is your password correct?')
    ...
    def bookmarks(self):
        """Returns a list the user's bookmarks

        Generates a tuple of work_id,title,author,fandom,warnings,relationships,characters,freeforms,works,words,chapters,comments,kudos,bookmarks,hits,pubdate
        ...
        for page_no in itertools.count(start=1):
            ...
            for li_tag in ol_tag.findAll('li', attrs={'class': 'blurb'}):
                try:
                    ...
                    work_id=str(li_tag.find('h4', attrs={'class':'heading'}).find('a').attrs['href'].replace('/works/', '')).replace('/series/', '')
                    ...
                    if chapters:
                        ...
                        #if we have no chapters, it's a series instead
                        chapters = 0
                    ...
                    pubdate_str=li_tag.find('p',attrs={'class','datetime'}).contents[0]
                    pubdate = datetime.strptime(pubdate_str, '%d %b %Y').date()
                    yield work_id,title,author,fandom,warnings,relationships,characters,freeforms,works,words,chapters,comments,kudos,bookmarks,hits,pubdate
                    ...
                    #       <div class="mystery header picture module">
                        ...
                        self.deleted+=1
                        pass
                    elif 'deleted' in li_tag.contents[1]:
                        self.deleted+=1
                        pass
                    else:
                        self.deleted+=1
